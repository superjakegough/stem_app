			...
			<v-card>
				<v-toolbar flat class="tabletoolbar">
					...
					<v-flex xs4 class="text-xs-right">
						<v-btn flat color="accent" v-on:click.native="createJob">Create</v-btn>
					</v-flex>
				</v-toolbar>
		...
		<v-dialog v-model="dialog" max-width="290">
			<v-card>
				...
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn color="primary" flat @click.native="dialog = false">Cancel</v-btn>
					<v-btn color="primary" flat v-on:click.native="deleteJob">Accept</v-btn>
				</v-card-actions>
			</v-card>
		</v-dialog>
		<v-snackbar v-model="failed" color="error">Failed to delete job!<v-btn dark flat @click="failed = false">Close</v-btn></v-snackbar>
	</v-layout>
</template>
